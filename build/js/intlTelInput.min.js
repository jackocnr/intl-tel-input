...
! function(a) {
  ...
  function e(b, c) {
    this.element = b, this.options = a.extend({}, h, c), this._defaults = h, this.ns = "." + f + g++, this.isGoodBrowser = Boolean(b.setSelectionRange), this.hadInitialPlaceholder = Boolean(a(b).attr("placeholder")), this._name = f, this.init()
  }
  var f = "intlTelInput",
    g = 1,
    h = {
  ...
  a(b).load(function() {
    ...
    init: function() {
      var b = this;
      "auto" == this.options.defaultCountry ? a.get("http://ipinfo.io", function(a) {
        b.options.defaultCountry = a && a.country ? a.country.toLowerCase() : "", b.ready()
      }, "jsonp") : this.ready()
    },
    ready: function() {
      this.options.nationalMode && (this.options.autoHideDialCode = !1), navigator.userAgent.match(/IEMobile/i) && (this.options.autoFormat = !1), b.innerWidth < 500 && (this.options.responsiveDropdown = !0), this._processCountryData(), this._generateMarkup(), this._setInitialState(), this._initListeners()
    },
    ...
    _setInstanceCountryData: function() {
      ...
      if (this.options.onlyCountries.length)
        for (this.countries = [], a = 0; a < this.options.onlyCountries.length; a++) {
          var b = this._getCountryData(this.options.onlyCountries[a], !0, !1);
          b && this.countries.push(b)
        } else this.countries = k;
      for (this.countryCodes = {}, a = 0; a < this.countries.length; a++) {
        var c = this.countries[a];
    ...
    _setPreferredCountries: function() {
      ...
      for (var a = 0; a < this.options.preferredCountries.length; a++) {
        var b = this.options.preferredCountries[a],
          c = this._getCountryData(b, !1, !0);
        c && this.preferredCountries.push(c)
      }
    },
    _generateMarkup: function() {
      this.telInput = a(this.element), this.telInput.wrap(a("<div>", {
      ...
      }).appendTo(b), this.preferredCountries.length && (this._appendListItems(this.preferredCountries, "preferred"), a("<li>", {
        "class": "divider"
    ...
    _setInitialState: function() {
      ...
      if (this._getDialCode(a)) this._updateFlagFromNumber(a);
        ...
        var b;
        b = this.options.defaultCountry ? this._getCountryData(this.options.defaultCountry, !1, !1) : this.preferredCountries.length ? this.preferredCountries[0] : this.countries[0], this._selectFlag(b.iso2), a || this._updateDialCode(b.dialCode, !1)
      }
    ...
    _initListeners: function() {
      ...
      var d = this.telInput.closest("label");
      d.length && d.on("click" + this.ns, function(a) {
        c.countryList.hasClass("hide") ? c.telInput.focus() : a.preventDefault()
      });
      var e = this.selectedFlagInner.parent();
      e.on("click" + this.ns, function() {
        c.countryList.hasClass("hide") && !c.telInput.prop("disabled") && c._showDropdown()
      }), this.options.utilsScript && (j ? this.loadUtils() : a(b).load(function() {
        c.loadUtils()
      }))
    },
    _initKeyListeners: function() {
      ...
      this.options.autoFormat && this.telInput.on("keypress" + this.ns, function(c) {
        if (c.which >= i.SPACE && !c.metaKey && b.intlTelInputUtils) {
          c.preventDefault();
          var d = c.which >= i.ZERO && c.which <= i.NINE || c.which == i.PLUS,
            ...
            h = g ? a.telInput.val().length < g : !0;
          if (h && (d || f)) {
            var j = d ? String.fromCharCode(c.which) : null;
            a._handleInputKey(j, !0)
          }
          d || a.telInput.trigger("invalidkey")
        }
      }), this.telInput.on("keyup" + this.ns, function(c) {
        if (c.which == i.ENTER);
        else if (a.options.autoFormat && b.intlTelInputUtils) {
          var d = c.which == i.CTRL || c.which == i.CMD1 || c.which == i.CMD2,
            e = a.telInput[0],
            f = a.isGoodBrowser && e.selectionStart == e.selectionEnd,
            g = a.isGoodBrowser && e.selectionStart == a.telInput.val().length;
          if ((c.which == i.DEL && !g || c.which == i.BSPACE || d && f) && a._handleInputKey(null, d && g), !a.options.nationalMode) {
            var h = a.telInput.val();
            if ("+" != h.substr(0, 1)) {
              var j = a.isGoodBrowser ? e.selectionStart + 1 : 0;
              a.telInput.val("+" + h), a.isGoodBrowser && e.setSelectionRange(j, j)
            }
          }
    ...
    _handleInputKey: function(a, b) {
      ...
      this.setNumber(c, b), this.isGoodBrowser && (e && (d = this.telInput.val().length), f.setSelectionRange(d, d))
    },
    _initFocusListeners: function() {
      ...
      this.options.autoHideDialCode && this.telInput.on("mousedown" + this.ns, function(b) {
        ...
        a.telInput.data("focusVal", c), a.options.autoHideDialCode && (c || (a._updateVal("+" + a.selectedCountryData.dialCode, !0), a.telInput.one("keypress.plus" + a.ns, function(c) {
          if (c.which == i.PLUS) {
    ...
    _updateVal: function(a, c) {
      var d;
      if (this.options.autoFormat && b.intlTelInputUtils) {
        d = intlTelInputUtils.formatNumber(a, this.selectedCountryData.iso2, c);
    ...
    _updateFlagFromNumber: function(a) {
      ...
      if (b) {
        var c = this.countryCodes[this._getNumeric(b)],
          d = !1;
        if (this.selectedCountryData)
          for (var e = 0; e < c.length; e++) c[e] == this.selectedCountryData.iso2 && (d = !0);
        if (!d || this._isUnknownNanp(a, b))
          for (var f = 0; f < c.length; f++)
            if (c[f]) {
              this._selectFlag(c[f]);
              break
            }
      }
    },
    ...
    _selectFlag: function(a) {
      this.selectedCountryData = this._getCountryData(a, !1, !1), this.selectedFlagInner.attr("class", "flag " + a);
      var b = this.selectedCountryData.name + ": +" + this.selectedCountryData.dialCode;
      this.selectedFlagInner.parent().attr("title", b), this._updatePlaceholder();
      var c = this.countryListItems.children(".flag." + a).first().parent();
      this.countryListItems.removeClass("active"), c.addClass("active")
    },
    _updatePlaceholder: function() {
      if (b.intlTelInputUtils && !this.hadInitialPlaceholder) {
        var a = this.selectedCountryData.iso2,
          c = this.options.numberType ? intlTelInputUtils.numberType[this.options.numberType] : intlTelInputUtils.numberType.FIXED_LINE,
          d = intlTelInputUtils.getExampleNumber(a, this.options.nationalMode, c);
        this.telInput.attr("placeholder", d)
      }
    },
    _selectListItem: function(a) {
      var b = a.attr("data-country-code");
      this._selectFlag(b), this._closeDropdown(), this._updateDialCode(a.attr("data-dial-code"), !0), this.telInput.trigger("change"), this.telInput.focus()
    },
    ...
    _updateDialCode: function(b, c) {
      ...
      if (b = "+" + b, this.options.nationalMode && "+" != e.substr(0, 1)) d = e;
      else if (e) {
    ...
    destroy: function() {
      this._closeDropdown(), this.telInput.off(this.ns), this.selectedFlagInner.parent().off(this.ns), this.telInput.closest("label").off(this.ns);
      var a = this.telInput.parent();
      a.before(this.telInput).remove()
    },
    getCleanNumber: function() {
      return b.intlTelInputUtils ? intlTelInputUtils.formatNumberE164(this.telInput.val(), this.selectedCountryData.iso2) : ""
    },
    ...
    loadUtils: function(b) {
      var c = b || this.options.utilsScript;
    ...
    setNumber: function(a, b) {
      this.options.nationalMode || "+" == a.substr(0, 1) || (a = "+" + a), this._updateFlagFromNumber(a), this._updateVal(a, b)
    },
