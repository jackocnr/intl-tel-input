...
function formatNumber(val, countryCode, addSuffix, allowExtension, isAllowedKey) {
  try {
    ...
    for (var i = 0; i < clean.length; i++) {
      ...
      // if adding this char didn't change the length, or made it smaller (and there's no longer any spaces): that means that formatting failed which means the number was no longer a potentially valid number, so if we're allowing extensions: assume the rest is the ext provided the number is not still classified as too short
