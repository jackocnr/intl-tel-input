...
function formatNumber(val, countryCode, addSuffix, allowExtension) {
  try {
    ...
    if (addSuffix) {
      ...
      } else if (allowExtension && result && test.length <= result.length && test.indexOf(" ") == -1) {
        // else if the next digit would break the formating, and we're allowing extensions, add the suffix
        result += extSuffix;
      }
    } else if (next == -1) {
      // we still need to do this even if !addSuffix
      result += extSuffix;
    }

    // if the number contains an extension
    if (next == -1) {
      result += clean.substring(i, clean.length);
    }
    return result;
  } catch (e) {
    return val;
