...
function isValidNumberForRegion(number, countryCode) {
  try {
    var phoneUtil = i18n.phonenumbers.PhoneNumberUtil.getInstance();
    var numberObj = phoneUtil.parseAndKeepRawInput(number, countryCode);
    return phoneUtil.isValidNumberForRegion(numberObj, countryCode);
  } catch (e) {
    return false;
  }
}
...
goog.exportSymbol('intlTelInputUtils.isValidNumberForRegion', isValidNumberForRegion);
// enums
