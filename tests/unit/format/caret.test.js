...
import { computeNewCursorPosition } from "../../../src/js/format/caret.ts";

describe("format/caret computeNewCursorPosition", () => {
  test("returns 0 when at start and backspacing", () => {
    expect(computeNewCursorPosition(1, "+1 23", 0, false)).toBe(0);
  });
  ...
  test("moves to just after final relevant char (not skipping following space)", () => {
    ...
    expect(computeNewCursorPosition(3, "+12 3", 4, false)).toBe(3);
  });

  test("delete forwards places caret before next relevant char", () => {
    // relevant chars = 2, expect position right before 3rd relevant
    expect(computeNewCursorPosition(2, "+12 3", 2, true)).toBeGreaterThanOrEqual(2);
  });
  ...
  test("forward-delete at start preserves position 0", () => {
    ...
    expect(computeNewCursorPosition(0, "+12 3", 0, true)).toBe(0);
  });

  test("returns end of string when relevantChars exceeds total", () => {
    expect(computeNewCursorPosition(99, "+12 34", 6, false)).toBe("+12 34".length);
  });
  ...
  test("caret lands after last relevant char when at end", () => {
    ...
    expect(computeNewCursorPosition(9, formatted, formatted.length, false)).toBe(formatted.length);
  });

  test("caret skips over formatting chars when counting relevant", () => {
    // "+1 (234) 5" - find position after 5 relevant chars (+,1,2,3,4)
    expect(computeNewCursorPosition(5, "+1 (234) 5", 7, false)).toBe(7); // after "4" at index 6, returns 7
  });
  ...
  test("backspace from position 1 traverses normally (not early-return)", () => {
    ...
    expect(computeNewCursorPosition(1, "+12", 1, false)).toBe(1);
  });
  ...
  test("delete-forwards counting accounts for the '+' as relevant", () => {
    ...
    expect(computeNewCursorPosition(1, "+12 34", 1, true)).toBe(1);
  });

  test("handles empty formatted value", () => {
    expect(computeNewCursorPosition(0, "", 0, false)).toBe(0);
    expect(computeNewCursorPosition(3, "", 0, false)).toBe(0);
  });

  test("handles formatted value of only formatting chars", () => {
    expect(computeNewCursorPosition(0, "   ", 0, false)).toBe(0);
    expect(computeNewCursorPosition(2, "   ", 2, false)).toBe(3);
  });
});
