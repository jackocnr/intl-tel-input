...
describe("format/caret translateCursorPosition", () => {
  ...
  test("forward-delete at start preserves position 0", () => {
    ...
    expect(translateCursorPosition(0, "+12 3", 0, true)).toBe(0);
  });

  test("returns end of string when relevantChars exceeds total", () => {
    expect(translateCursorPosition(99, "+12 34", 6, false)).toBe("+12 34".length);
  });
  ...
  test("caret lands after last relevant char when at end", () => {
    ...
    expect(translateCursorPosition(9, formatted, formatted.length, false)).toBe(formatted.length);
  });

  test("caret skips over formatting chars when counting relevant", () => {
    // "+1 (234) 5" - find position after 5 relevant chars (+,1,2,3,4)
    expect(translateCursorPosition(5, "+1 (234) 5", 7, false)).toBe(7); // after "4" at index 6, returns 7
  });
  ...
  test("backspace from position 1 traverses normally (not early-return)", () => {
    ...
    expect(translateCursorPosition(1, "+12", 1, false)).toBe(1);
  });
  ...
  test("delete-forwards counting accounts for the '+' as relevant", () => {
    ...
    expect(translateCursorPosition(1, "+12 34", 1, true)).toBe(1);
  });

  test("handles empty formatted value", () => {
    expect(translateCursorPosition(0, "", 0, false)).toBe(0);
    expect(translateCursorPosition(3, "", 0, false)).toBe(0);
  });

  test("handles formatted value of only formatting chars", () => {
    expect(translateCursorPosition(0, "   ", 0, false)).toBe(0);
    expect(translateCursorPosition(2, "   ", 2, false)).toBe(3);
  });
});
