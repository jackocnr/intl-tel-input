...
// English names for the language codes we ship locale packs for. Used as a
...
function resolveLanguageName(code) {
  ...
  const label = localeDisplayNames ? localeDisplayNames.of(tag) : null;
...
export function createLocaleOptionLabels(languageCodes) {
  const labels = {};
...
export async function resolveLocaleSelection(value, { localeDirHash }) {
  ...
  // additional locale pack (and keep the generated init code simpler).
  if (code.toLowerCase() === "en") {
    return null;
  }

  try {
    const mod = await import(`/intl-tel-input/js/locale/${code}.js?${localeDirHash}`);
