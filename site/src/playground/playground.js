// constants injected by grunt/template.js
...
const UTILS_PATH = "<%= cacheBust('/intl-tel-input/js/utils.js') %>";
const I18N_DIR_HASH = "<%= getDirHash('/intl-tel-input/js/i18n') %>";
...
function toInitOptions(state) {
  ...
  if (state.loadUtils) {
    opts.loadUtils = () => import(UTILS_PATH);
  }
...
async function resolveI18nSelection(value) {
  ...
  try {
    const mod = await import(`/intl-tel-input/js/i18n/${code}/index.js?${I18N_DIR_HASH}`);
...
function renderInitCodeFromState(state) {
  ...
  if (initCodeEl && window.hljs && typeof window.hljs.highlightElement === "function") {
